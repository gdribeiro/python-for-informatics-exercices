...
for line in fhandle:
    ...
    # Here we will combine the searching with extracting the pattern
    # IMPORTANT! The . inside the brakets means only a . and not a wildcard as it means outside
    # As an example we will extract only the floats from lines like
    # X-DSPAM-Confidence: 0.8475
    # X-DSPAM-Probability: 0.0000
    # X-DSPAM-Confidence: 0.6178
    # X-DSPAM-Probability: 0.0000

    relst = re.findall('^X.*: ([0-9.]+)', line)
    if len(relst) > 0:
        print relst
